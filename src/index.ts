  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    bot.on("inline_query", async (ctx) => {
      ...
      const spoilerTitle = tooManyEntities
        ? "1) Send with Spoiler (Too many spoiler entities: only first 100 spoilers and their corresponding text will be sent)"
        : "1) Send with Spoiler";
    ...
    bot.on("message:text", async (ctx) => {
      ...
      if (truncatedResult.truncated) {
        await ctx.reply(
          "Your message exceeded Telegram's 100-entity limit. Only the first 100 spoilers and their corresponding text were sent.",
        );
      }
    });
